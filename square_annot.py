...
for i in range(1,2+1):
	save_dir = os.path.join(cfg.DATA_DIR,'underwood_dataset','apples','square_annotations'+str(i)+'')
	...
	for file in Files:
		...
		#Square_annotations1
		if i == 1:	
			xmin = circles[:,1] - circles[:,3] 
			xmax = circles[:,1] + circles[:,3] 
			ymin= circles[:,2] - circles[:,3] 
			ymax = circles[:,2] + circles[:,3]
		#Square_annotations2
		if i == 2:
			xmin = circles[:,2] - circles[:,3] 
			xmax = circles[:,2] + circles[:,3] 
			ymin= circles[:,1] - circles[:,3] 
